...
Merged: Validation Logic from Script 1 + Agent Maintenance from Script 2
All Submenus Functional and Inlined
#>
        ...
        switch ($mainChoice.ToUpper()) {
            ...
            "Q" {
                ...
                if ($confirm -eq 'Y') {
                    ...
                    Write-Host "Data deleted. Exiting..." -ForegroundColor Green
                    exit
                }
            }
...
function Run-AgentInstallTool {
    ...
    $csvLogFile = "$script:exportPath\AgentMaintenance-$timestamp-$hostname.csv"
    $textLogFile = "$script:exportPath\AgentMaintenance-FullOutput-$timestamp-$hostname.txt"
    ...
    function Run-Uninstall {
        ...
        if (Test-Path $uninstallPath) {
            try {
                ...
                $backupPath = "$script:exportPath\UninstallScriptBackup-$timestamp-$hostname.txt"
