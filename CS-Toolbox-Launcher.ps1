...
function Show-LauncherMenu {
    ...
    Write-Host "╚════════════════════════════════════════════════════╝" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "🧩 [1] Validation Tool A             → Office, Drivers, Roaming"
    Write-Host "📦 [2] Validation Tool B             → Extensions, TLS, Patches"
    Write-Host "🌐 [3] Network Testing Tool          → SMB, TLS 1.0, Nmap Scans"
    Write-Host "🛠  [4] Agent Management Tool        → Install, Status, Clear Jobs"
    Write-Host "👥 [5] Active Directory Tool         → Users, Groups, OUs, GPOs"
    Write-Host "🧪 [6] Utilities Tool                → Dependency Walker, Cleanup"
    Write-Host ""
...
do {
    ...
    switch ($choice) {
        "1" {
            Launch-Tool -toolUrl "https://raw.githubusercontent.com/dmooney-cs/prod/refs/heads/main/ValidationTool-Collection%20A.ps1"
        }
        "2" {
            Launch-Tool -toolUrl "https://raw.githubusercontent.com/dmooney-cs/prod/refs/heads/main/ValidationTool-Collection%20B.ps1"
        }
        "3" {
